...
const suitAccent: Record<string, string> = {
  Wands: 'from-rose-300 via-red-200 to-orange-300',
...
export function TarotCardArt({ card, variant, compact = false }: TarotCardArtProps) {
  const accent = card.suit ? suitAccent[card.suit] : 'from-rose-300 via-red-200 to-orange-300';
  ...
  if (variant === 'back') {
    return (
      <div className="relative h-full overflow-hidden border border-rose-200/20 bg-[radial-gradient(circle_at_top,rgba(244,63,94,0.22),transparent_45%),linear-gradient(145deg,#0f172a_0%,#020617_100%)] shadow-[0_0_40px_rgba(244,63,94,0.08)]">
        <div className="absolute inset-0 opacity-70" style={{ backgroundImage: 'radial-gradient(circle at 1px 1px, rgba(244,63,94,0.22) 1px, transparent 0)', backgroundSize: '18px 18px' }} />
        <div className="absolute inset-0 border border-rose-100/10" />
      </div>
    );
  }

  return (
    <div className={`relative h-full overflow-hidden border ${variant === 'public' ? 'border-white/10 bg-white/5 backdrop-blur' : 'border-white/10 bg-gradient-to-br from-rose-100 via-red-50 to-orange-200 text-slate-950 shadow-[0_0_35px_rgba(255,255,255,0.18)]'}`}>
