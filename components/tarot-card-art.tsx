...
export function TarotCardArt({ card, variant, compact = false }: TarotCardArtProps) {
  ...
  return (
    <div className={`relative overflow-hidden rounded-[2rem] border p-5 ${variant === 'public' ? 'border-white/10 bg-white/5 backdrop-blur' : 'border-white/10 bg-gradient-to-br from-amber-100 via-amber-50 to-orange-200 text-slate-950 shadow-[0_0_35px_rgba(255,255,255,0.18)]'}`}>
      ...
      <div className="relative flex h-full flex-col">
        ...
        <div className="mt-5 rounded-[1.5rem] border border-current/10 bg-current/5 p-4">
          ...
          <div className={`mt-4 ${typographyScale} font-semibold leading-tight ${variant === 'public' ? 'text-slate-100' : 'text-slate-950'}`}>
            {card.name}
          </div>
        ...
        <div className={`mt-5 rounded-[1.5rem] border p-4 ${variant === 'public' ? 'border-white/10 bg-black/20 text-slate-200' : 'border-slate-900/10 bg-white/55 text-slate-700'}`}>
          <p className="text-xs uppercase tracking-[0.35em] text-amber-800/80">Meaning</p>
          <p className="mt-2 text-sm leading-6">{variant === 'public' ? 'This card was revealed for a public reading.' : 'Reveal this card to uncover its role in the spread.'}</p>
        </div>
      </div>
    </div>
  );
